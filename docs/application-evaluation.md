...
|---|---|---|---|
| 1 | **`CMP-140` cross-border transfer** | The topology is decided — one central deployment — which **removes** the arrangement in which nothing crossed a border and makes this sharper, not softer. Every non-EU jurisdiction now needs a lawful basis before its region is declared | Legal decision, and now the one that gates go-live for 7 of 21 entities. The code fails closed: `SERVED_REGIONS` defaults to the home region alone and the self-test names anything stranded outside it |
