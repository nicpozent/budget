...
*Last reviewed: v1.3, `claude/file-review-8a42qx`. 0 lint errors,
...
|---|---|---|---|---|
| 1 | **Functional coverage** (vs `SPEC.md`) | ★★★★☆ | Everything previously listed, plus the four gaps closed: **`FR-040`** idempotent ledger batch ingest with stored rejects and residency enforcement; **`FR-051`** ordered approval stages with role and threshold conditions that actually gate; **`FR-005`** immutable published template versions with in-flight budgets pinned; **`FR-033`** next year's depreciation as derived read-only opex lines. Screens added for trend, FX history, allocations, scenarios, drivers, the approval chain and template versions — all 16 views axe-clean. **`FR-080`** budget versions, scenarios and a rolling forecast, with a foreign key and a lock behind them; **`FR-020`** derived driver trees | No live feed is connected to the `FR-040` endpoint and there is no service principal to run it; no formula engine, deliberately (ADR 0007) |
| 2 | **Architecture & modularity** | ★★★★★ | npm workspaces; pure `shared` package (no I/O) so the matrix and money type are directly testable; **routes split by domain, services by layer** (ADR-0006), keeping `guard.ts`/`pool.ts`/`audit.ts` singular and reviewable; FR-051's gating logic is a pure function tested without a request; HLD + LLD + building blocks + 7 ADRs. Layering is asserted rather than assumed: no service imports a route, `shared` imports nothing upward, and the client never reaches into the API package | **`admin.ts` drifted back into being the file ADR-0006 exists to forbid** — 712 lines and four unrelated domains, held together only by the fact that an administrator performs all of them. Split into `reference`, `drivers`, `governance` and `operations`; no route module now exceeds 600 lines. Worth recording because the ADR named the failure mode and it happened anyway: a module named after a *role* will accept anything that role can do. **And then it happened a second time on the client** — `components/views.tsx` held seven unrelated screens in 641 lines while five other views each had their own file. Same failure, blander name: a module named after nothing accepts anything. Split per view |
| 3 | **Frontend engineering** | ★★★★★ | React 18 + TS strict + Vite 6; external CSS tokens (no inline styles — the CSP forbids them); 0 lint errors; deterministic asset names; **route-level splitting** (per-view chunks a role may never fetch) with the split **asserted in a real browser under the real CSP**, because a dynamic import does not inherit the shell nonce; shared store on `useSyncExternalStore` replacing per-view refetching | Splitting is per view, not per route segment — there is no router. Deliberate: the client owns one navigation state. **The per-view claim was only three-quarters true until this revision**: the unit of a chunk is a file, and seven views shared one, so a budget owner downloaded the governance and audit screens. Seven files now, seven chunks of 1.8–3.4 kB |
...
| 5 | **Authorization model** | ★★★★★ | SPEC §5 matrix as frozen data; **an undeclared route throws at registration** so it cannot reach a running server; capability and entity scope as separate axes; 404-not-403 on out-of-scope reads; **270 assertions covering every role × capability pair** (30 capabilities × 9 roles), generated from the matrix so a new capability without a probe fails the suite | — |
...
| 9 | **Security & hardening** | ★★★★☆ | Nonce CSP with no `unsafe-inline`; full header set; CSRF token + origin check + `SameSite`; rate limiting incl. per-route; bound-parameter SQL with an allow-list for identifiers and **lint rules that block the alternatives**; formula-injection guard verified by inflating a real workbook; step-up on irreversible actions; config fails closed in production; TLS to the database with `verify-full`; **15-minute idle timeout now enforced** rather than merely recorded; **ZAP baseline in CI** with injection and XSS rules set to fail; **an automated attack suite** (`test/pentest.test.ts`, 17 tests) that sweeps *every registered route* for an unauthenticated or under-privileged response, plus prototype pollution, oversized bodies, path traversal, method override, header injection, post-logout token reuse and error-message leakage; **Sigstore signing and SLSA provenance** on the image | **Still no penetration test** (`SEC-041`), and the attack suite is not one: it tests the attacks someone thought of, which is the half a pen test is least valuable for. What it changes is the regression story — the sweep reads the route table from the running instance, so an endpoint added without authorisation fails the build rather than waiting for the next engagement. Verified by planting one: marking `/api/entities` public fails the gate. The DAST stage still runs with DEV_AUTH on so the scanner gets past the front door, which remains the one way the scanned system differs from production. `pentest-scope.md` now tells the tester which objectives are already automated and which five are worth the window — the ones about *sequences* of legitimate requests, where no scanner has a model of what the system is for |
...
| 11 | **Non-production data** | ★★★★☆ | Synthetic seed by default; offline anonymiser outside `packages/`; CI refuses any import of the workbook; **the anonymiser reports its own k-anonymity failure** rather than overclaiming | The real extract is still in the repository (`osint-exposure.md` §1); the anonymised fixture is pseudonymous, not anonymous. **`SEED_MODE=anonymised` was broken for a commit and nothing said so** — the harness always seeds synthetically, so the supported mode CI does not exercise is the one that drifted when `entities` gained a country. Now a generator-to-loader round trip, with the guard's own failure asserted |
| 12 | **Accessibility (WCAG 2.2 AA)** | ★★★★☆ | **WCAG 2.2.1 now met**: the session warns two minutes before the idle timeout with a Continue action, and says so plainly when it is the absolute TTL that cannot be extended. | axe across **16 views** in a real browser; contrast asserted arithmetically for both themes; type-scale floor parsed from tokens; non-colour cue beside every status; real table semantics; keyboard-operable scroll regions. **VPAT 2.5 / EN 301 549 report** criterion by criterion, marking which claims are verified and which are code-reading. **NFR-010 catalogue**: every client string typed, with a test that fails on JSX text bypassing it — it found 120 — and a second gate over `aria-label`, `title` and `placeholder`, **which found nine accessible names hardcoded in English**: the grid's checkboxes, the line drawer, the audit search box. Invisible to axe, which checks that a name exists rather than what language it is in, and invisible to the text gate, which matches between tags. A Swedish screen-reader user heard every one of them. **WCAG 4.1.3 fixed** while writing the VPAT: banners were not live regions. **A screen-reader gate that reads the browser's accessibility tree** (`test/screenreader.test.ts`) — region and control names, heading outline, landmark count, and focus moving into the line drawer and back out — plus axe's `best-practice` set, reported separately | Still ★★★★ for the same reason: **no assistive-technology user has tested this**, and reading the accessibility tree is not the same as using one. But the distance between "axe passes" and "navigable" turned out to be four defects wide — 25 scroll regions announcing "group" and nothing else, a drawer whose focus call had never executed, nine empty column headers, five English captions — and all four are now gated. Automated checks catch roughly a third of WCAG issues. six locales, none native-reviewed. A hover-contrast defect found this revision — the `background` shorthand on `:hover` wiped every primary button's gradient, leaving near-black text on grey — suggests other state-dependent contrast problems automated tooling cannot see |
...
| 14 | **Testing** | ★★★★★ | 676 tests, most against a **real PostgreSQL** (throwaway DB per run) because constraints, triggers and grants are half the controls; authz matrix, security regressions, invariant properties, browser a11y, operations; **real defects found by the suite and by review during build** and each now has a regression test **Runtime self-test**: 17 read-only checks over live data, runnable from the Operations view, PowerShell, `npm run selftest` or any scheduler, with tests that break the system on purpose to prove each check can fail. **Load test at monthly × three-version scale** (`npm run loadtest`). **Mutation testing** of `packages/shared` (`npm run mutate`, nightly, threshold 90): 73 mutants, all killed | **The load test found a real NFR-001 breach, and the breach is now fixed**: at 30k period rows four of six report routes exceeded the 300 ms p95 budget, trend worst at 557 ms; the fold moved into SQL and the worst p95 is 195 ms with throughput up from 88 to 171 requests a second. The equivalence of the two implementations is asserted over the whole seeded dataset rather than assumed, and doing it turned up two more defects (a drifted EUR rate in the fixture, and a load-test tool that measured with the rate limiter on). **The mutation run named six under-tested validators on its first pass** — the `Money` range boundary, the control-character guards, the decimal-scale bound, the negative-rate refusal, the percentage limits and the driver-factor limits — each covered by a line the suite ran and no assertion depended on. All now asserted. Four mutants are marked equivalent in the source, with the reason, and are reported outside the score rather than folded into it |
...
| 22 | **Documentation** | ★★★★☆ | HLD, LLD, building blocks, 6 ADRs, threat model, security hardening, observability, accessibility, VPAT, retention, secrets, Postgres TLS, Sweden compliance, DPIA, RoPA, lawful basis, privacy notice, pentest scope, OSINT, user stories, per-role user guide with screenshots, SoW with 28 flow diagrams. **`test/docs.test.ts` now gates the figures**: capability × role counts, view count, self-test checks, production dependencies, migrations named, and every source path cited | Dropped to ★★★★ on evidence: twenty-two figures across nine documents were stale — "203 assertions" and "360 tests" long after both were wrong, including in the Statement of Work, and the LLD's module tree still listed `admin.ts` two commits after it was deleted. All corrected and the structural ones are now gated, but **the user guide still documents 12 of 16 screens** — Scenarios, Drivers, Approval chain and Template have no section and no screenshot |
...
cannot reach a running server. The 270 assertions are generated *from the
