...
| [0006](../adr/0006-module-structure.md) | Routes by domain, services by layer | The domain grew past what one `admin.ts` could hold, twice — the second time the ADR's own warning was ignored and `admin.ts` was split into reference, drivers, governance and operations. The security-relevant code stays in three reviewable files |
