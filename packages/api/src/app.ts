...
import { registerDriverRoutes } from './routes/drivers.ts';
...
export async function buildApp({ db, config }: AppDeps): Promise<FastifyInstance> {
  ...
  await registerReferenceRoutes(app, db, config);
  await registerDriverRoutes(app, db, config);
  await registerGovernanceRoutes(app, db, config);
  await registerOperationsRoutes(app, db, config);
